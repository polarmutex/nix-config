...
local function path_component()
    ...
    if ft == "git" or vim.fn.win_gettype(winid) == "command" then
        return ""
    end
...
-- vim.o.winbar = "%{%v:lua.require('polar.core.ui.winbar').render()%}"

vim.api.nvim_create_autocmd("BufWinEnter", {
    group = vim.api.nvim_create_augroup("polar/winbar", { clear = true }),
    callback = function()
        if not vim.api.nvim_win_get_config(0).zindex then
            vim.wo.winbar = "%{%v:lua.require('polar.core.ui.winbar').render()%}"
        end
    end,
})
