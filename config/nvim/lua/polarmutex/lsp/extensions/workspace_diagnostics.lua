...
local filter_to_severity_limit = function(severity, diagnostics)
    ...
    if not filter_level then return diagnostics end

    return
        vim.tbl_filter(function(t) return t.severity == filter_level end, diagnostics)
end
...
M.handler = function(err, method, result, client_id, bufnr, config)
    ...
    if not M.diagnostic_cache[client_id] then M.diagnostic_cache[client_id] = {} end

    local diagnostics = result.diagnostics

    local counts = {}
    for _, severity in ipairs(DiagnosticSeverity) do
        counts[to_severity(severity)] = vim.lsp.diagnostic.get_count(bufnr, severity,
                                                                     client_id)
    end

    M.diagnostic_cache[client_id][bufnr] = {diagnostics = diagnostics, counts = counts}
end

M.get_count = function(bufnr, severity)
    if bufnr == 0 or not bufnr then bufnr = vim.api.nvim_get_current_buf() end
...
M.set_qf_list = function(opts)
    ...
    for diag_bufnr, diags in pairs(diags_by_buffer) do
        ...
        elseif opts.severity_limit then
            diags_by_buffer[diag_bufnr] = filter_by_severity_limit(opts.severity_limit,
                                                                   diags)
        end
    end
    ...
    local insert_diag = function(diag_bufnr, diag)
        ...
        local col_ok, col = pcall(vim.lsp.util.character_offset, bufnr, row,
                                  pos.character)
        if not col_ok then col = pos.character end

        local line =
            (vim.api.nvim_buf_get_lines(bufnr, row, row + 1, false) or {""})[1] or ""
    ...
    for diag_bufnr, diags in pairs(diags_by_buffer) do
        for _, v in ipairs(diags) do insert_diag(diag_bufnr, v) end
    end

    vim.fn.setqflist({}, "r", {title = "LSP Diagnostics", items = item_list})

    if open_qflist then vim.cmd([[copen]]) end
end
...
function M.get(bufnr)
    ...
    for client_id, _ in pairs(vim.lsp.buf_get_clients(bufnr)) do
        for diag_bufnr, diag_cache in pairs(M.diagnostic_cache[client_id] or {}) do
            if not result[diag_bufnr] then result[diag_bufnr] = {} end
