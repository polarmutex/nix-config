...
local function install_path(lang) return vim.fn.stdpath("data") .. "/lspinstall/" ..
                                             lang end
...
function M.uninstall_server(lang)
    ...
    if os.execute("test -d " .. path) ~= 0 then error("server is not installed") end
...
function M.installed_servers()
    return vim.tbl_filter(function(key) return M.is_server_installed(key) end,
                          M.available_servers())
end
